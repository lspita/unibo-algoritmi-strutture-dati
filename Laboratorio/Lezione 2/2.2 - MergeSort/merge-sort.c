...
copy), Attribution,
<https://commons.wikimedia.org/w/index.php?curid=3429594>](von-neumann.jpg "John
von Neumann")
...
void merge(int *v, int p, int q, int r, int *buffer) {
  int i = p, j = q + 1, current = p;
  int stop_i = 0, stop_j = 0;
  while (!stop_i || !stop_j) {
    stop_i = i > q;
    stop_j = j > r;

    if (!stop_i && (stop_j || v[i] <= v[j])) {
      buffer[current] = v[i];
      i++;
    } else if (!stop_j && (stop_i || v[j] <= v[i])) {
      buffer[current] = v[j];
      j++;
    }
