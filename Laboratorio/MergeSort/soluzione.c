...
void merge(int *v, int p, int q, int r, int *buffer)
{
    ...
    /* Eseguiamo la merge di `v[p..q]` e `v[q+1..r]` in
       buffer[p..r] */
    while (i <= q && j <= r) {
        /* INVARIANTE: buffer[p..k-1] è ordinato e contiene i (k-p)
           valori minimi presenti nell'unione di `v[p..q]` e
           `v[q+1..r]` */
        if (v[i] <= v[j]) {
            buffer[k] = v[i];
            i++;
    ...
    /* Se i due sottovettori hanno dimensioni diverse, occorre
       completare la copia dei valori in coda al sottovettore più
       lungo all'interno di `buffer[]` */
    while (i <= q) {
        buffer[k] = v[i];
        i++;
        k++;
