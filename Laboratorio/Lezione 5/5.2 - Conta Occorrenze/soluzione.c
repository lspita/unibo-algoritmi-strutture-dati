...
Il sistema operativo Unix mette a disposizione dei comandi che
consentono di risolvere questo problema senza scrivere codice:

        cat the-time-machine.txt | tr -c a-zA-Z '\n' | sed '/^$/d' | tr A-Z a-z | sort | uniq -c

La catena di comandi sopra si chiama _pipeline_, dove l'output di
ciascun comando viene usate come input dal successivo. Il significato
dei vari comandi è il seguente:

- `cat the-time-machine.txt` legge il contenuto del file
  `the-time-machine.txt` e lo stampa su standard output

- `tr -c a-zA-Z '\n'` trasforma tutte le occorrenze di caratteri NON
  alfabetici in "a capo". L'output avrà quindi una parola per riga,
  eventualmente con molte righe vuote corrispondenti alle sequenze di
  simboli non alfabetici nell'input

- `sed '/^$/d'` elimina le righe vuote

- `tr A-Z a-z` trasforma tutte le lettere maiuscole in minuscole

- `sort` ordina le righe

- `uniq -c` stampa ciascuna parola una volta sola, con il conteggio
  del numero di occorrenze

Se vogliamo ordinare le parole per frequenza decrescente (prima quelle
più frequenti) possiamo aggiungere `| sort -nr` alla fine, per
ordinare il risultato numericamente (`-n`) in senso contrario (`-r`,
gioè dal più grande al più piccolo); scopriamo quindi che le dieci
parole più frequenti ne "La Guerra dei Mondi" sono:
...
int read_word(FILE *f, char *s)
{
    ...
    /* Legge la parola; si noti che se siamo usciti dal ciclo
       precedente perché il file è terminato (c == EOF), il blocco
       seguente viene correttamente saltato. */
    while ((c != EOF) && isalpha(c)) {
        assert(i < WORDLEN-1);
        s[i] = tolower(c);
        i++;
...
BSTNode *insert(const char *s, BSTNode *t)
{
    if (t == NULL) {
        ...
        assert(result != NULL);
        result->s = (char*)malloc(strlen(s)+1);
...
/* Stampa il contenuto dell'albero `t` con una visita in-ordine
   (visita figlio sinistro; visita nodo; visita figlio destro);
   restituisce il numero di nodi visitati, che corrisponde al numero
   di parole distinte presenti nel file. */
int print(const BSTNode *t)
{
    if (t == NULL) {
        return 0;
    } else {
        int tree_size = print(t->left);
        printf("%s %d\n", t->s, t->count);
        tree_size++;
        tree_size += print(t->right);
        return tree_size;
    }
}

/* Calcola l'altezza dell'albero radicato in `t`; l'albero vuoto si
   assume di altezza -1. */
int height(const BSTNode *t)
{
    if (t == NULL)
        return -1;
    else {
        const int hl = height(t->left);
        const int hr = height(t->right);
        return 1 + (hl > hr ? hl : hr);
    }
}

/* Libera tutta la memoria occupata dal sottoalbero radicato in
   `node`. */
