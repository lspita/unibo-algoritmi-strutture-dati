...
![C. A. R. Hoare (Photograph by Rama, Wikimedia Commons, CC BY-SA 2.0 fr,
<https://commons.wikimedia.org/w/index.php?curid=15568323>)](Hoare.jpg "Sir
...
![Figura 2: Invariante della funzione `partition(v, start, end)` secondo lo
schema di Lomuto](partition-invariant.svg)

[^1]: Personalmente trovo lo [schema di
Lomuto](https://en.wikipedia.org/wiki/Quicksort#Lomuto_partition_scheme) poco
intuitivo; se dovessi implementare Quicksort a mano, probabilmente userei lo
      [schema di
...
- `nmemb` è il numero di elementi dell'array da ordinare (attenzione, **non** il
numero di byte!)
...
void sort(int *v, int n) { quicksort(v, 0, n - 1); }
